...
# Save setup
file_dir = 'data/inverse/test'
os.makedirs(file_dir, exist_ok=True)
file_name = 'ellipse_hole3'

# Load data (measured displacement)
sol_measured = onp.loadtxt('ellipse_hole3.txt') # (number of nodes, 3) in 3D
...
cen_exact = mid_point + np.array([20, 15])
rho_exact = np.array([cen_exact[0], cen_exact[1], 7.0, 3.0, np.pi/6]) # [x, y, l, l2, angle]
...
obj = []
k_list = np.arange(200, 281, dtype=float) # k1 values for the optimization
for k in k_list:
    k1 = k
    ...
    obj.append(results.fun)
    print('k:', k)

# Plot the optimization results
plt.figure(1, figsize=(10, 8))
plt.plot(k_list, obj, linestyle='-', linewidth=2, color='black')
plt.axhline(y=exact_obj, color='r', linestyle='--', label='J = %f' % exact_obj)
plt.xlabel(r"Sigmoid sharpness k", fontsize=20)
...
# Save
plt.savefig(os.path.join(file_dir, file_name, 'sigmoid_sharpness_graph.png'), dpi=300, bbox_inches='tight')