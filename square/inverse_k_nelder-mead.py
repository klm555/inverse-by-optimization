...
file_name = 'ellipse_hole2'

# Load data (measured displacement)
sol_measured = onp.loadtxt('ellipse_hole2.txt') # (number of nodes, 3) in 3D
...
class Geometry:
    ...
    def ellipse(self, problem) -> np.ndarray:   
        """
        Get indices of cells inside the ellipse (only 2D available).
        """   
        point_indicators = jax.nn.sigmoid(problem.k * # (+): outside, (-): inside  
...
class LinearElasticity(Problem):
    ...
    def set_params(self, params): # params = [x, y, z, r, h]
        ...
        full_params = inner_domain.ellipse(self)
...
# Objective Function for sharpness factor k
def J_k(k):
    # Run the optimization for the parameters x, y, l, l2, angle
    opt = ParamOptimizer(k)
    opt() # Explicitly call the instance to run the inner optimization
    # Data term
    param_difference = opt.results.x - rho_exact_normalized
    # Objective function
    J = 0.5 * np.linalg.norm(param_difference)**2
    return J
...
cen_exact = mid_point + np.array([30, 20])
rho_exact = np.array([cen_exact[0], cen_exact[1], 5.0, 2.0, np.pi/9]) # [x, y, l, l2, angle]
...
# For the final optimization step (to log and plot)
final_optimizer = ParamOptimizer(optimal_k.x)
final_results = final_optimizer()
...
x_unnormalized = unnormalize(final_results.x[0], x_bound[0], x_bound[1])
y_unnormalized = unnormalize(final_results.x[1], y_bound[0], y_bound[1])
l_unnormalized = unnormalize(final_results.x[2], l_bound[0], l_bound[1])
l2_unnormalized = unnormalize(final_results.x[3], l2_bound[0], l2_bound[1])
angle_unnormalized = unnormalize(final_results.x[4], angle_bound[0], angle_bound[1])
...
Total Iteration: {final_optimizer.counter - 1}
Sharpness: {optimal_k.x[0]}
Final Parameters: {final_param_unnormalized}
Exact Parameters: {rho_exact}
Final Objective Value: {final_results.fun}
...
with open(results_file_path, 'w') as f:
    f.write(log_info)
    f.write(f"\nOptimization Success: {final_results.success}\n")
    f.write(f"Optimization Message: {final_results.message}\n")
    f.write(f"Number of Function Evaluations: {final_results.nfev}\n")
    f.write(f"Number of Gradient Evaluations: {final_results.njev}\n")

print(f"Results saved to: {results_file_path}")

# Plot the optimization results
obj = onp.array(final_optimizer.outputs)
