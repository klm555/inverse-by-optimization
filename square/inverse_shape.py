...
class LinearElasticity(Problem):
    ...
    def set_params(self, params): # params = [x, y, z, r, h]
        ...
        inner_domain = Geometry(x, y, length=params[0], 
                                length2=params[1], angle=params[2], 
...
# Initial guess
rho_ini = np.array([l_bound[1]/2, l2_bound[1]/2, angle_bound[1]/2]) # it doesn't work with 0.0
# rho_ini = np.array([bound_sum[0]/2, bound_sum[1]/2, 0.1, 0.1, 1]) # it doesn't work with 0.0
l_ini_normalized = normalize(rho_ini[0], l_bound[0], l_bound[1])
l2_ini_normalized = normalize(rho_ini[1], l2_bound[0], l2_bound[1])
angle_ini_normalized = normalize(rho_ini[2], angle_bound[0], angle_bound[1])
rho_ini_normalized = np.array([l_ini_normalized, l2_ini_normalized, angle_ini_normalized])
...
rho_exact = np.array([5.0, 2.0, np.pi/3]) # [x, y, l, l2, angle]
...
angle_exact_normalized = normalize(rho_exact[2], angle_bound[0], angle_bound[1])
rho_exact_normalized = np.array([l_exact_normalized, l2_exact_normalized, angle_exact_normalized])
